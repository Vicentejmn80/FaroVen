 ...
 * Layout: cuerpo con scroll + pie fijo para que "Aceptar" siempre se vea.
 */
export function RoleSelectionScreen() {
  ...
  const canSubmit =
    Boolean(choice) && !busy && !(needsReason && reason.trim().length < 12)
  ...
  return (
    <div className="flex h-dvh max-h-dvh flex-col overflow-hidden bg-[#0B1626] text-[#F2F6FA]">
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 pt-[max(1.5rem,env(safe-area-inset-top))] lg:px-8">
        <div className="mx-auto w-full max-w-3xl pb-6">
          ...
          <p className="mb-5 max-w-2xl text-sm leading-relaxed text-[#8CA0B8] sm:mb-6">
      ...
      <div className="shrink-0 border-t border-[#1C2B40] bg-[#0B1626]/95 px-4 pt-3 backdrop-blur-md pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto w-full max-w-3xl">
          ...
          {!choice && (
            <p className="mt-2 text-center text-[11px] text-[#7690AC]">
