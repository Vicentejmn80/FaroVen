...
export function ScreenScaffold({ title, subtitle, onBack, action, children }: ScreenScaffoldProps) {
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="flex shrink-0 items-center gap-3 px-4 pt-safe pb-3">
        {onBack && (
      ...
      <div className="faro-scroll no-scrollbar px-4 pb-nav pt-3 lg:px-8 lg:pb-8">{children}</div>
    </div>
  )
}
