export const PIPELINE_STAGES = {
  ...
  AWAITING_CENTER_CONFIRMATION: 'awaiting_center_confirmation',
  ASSIGNED: 'assigned',
  ACCEPTED: 'accepted',
...
export const CASE_EVENT_TYPES = {
  ...
  CASE_OPENED_FOR_APPLICATIONS: 'case_opened_for_applications',
  CASE_AWAITING_CENTER: 'case_awaiting_center',
...
/**
 * Flujo operacional estabilizado:
 * - pending_review → open_for_applications (solicitar postulantes) O awaiting_center_confirmation
 * - ASIGNADO solo tras aceptar postulante o confirmar centro
 */
...
export const TRANSITION_TO_EVENT: Record<string, CaseEventType> = {
  [`${PIPELINE_STAGES.NUEVO}->${PIPELINE_STAGES.PENDING_REVIEW}`]: CASE_EVENT_TYPES.CASE_REVIEW_STARTED,
  [`${PIPELINE_STAGES.NUEVO}->${PIPELINE_STAGES.ARCHIVED}`]: CASE_EVENT_TYPES.CASE_DISMISSED,
  [`${PIPELINE_STAGES.PENDING_REVIEW}->${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}`]: CASE_EVENT_TYPES.CASE_OPENED_FOR_APPLICATIONS,
  [`${PIPELINE_STAGES.PENDING_REVIEW}->${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}`]: CASE_EVENT_TYPES.CASE_AWAITING_CENTER,
  ...
  [`${PIPELINE_STAGES.VALIDATING}->${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}`]: CASE_EVENT_TYPES.CASE_OPENED_FOR_APPLICATIONS,
  [`${PIPELINE_STAGES.VALIDATING}->${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}`]: CASE_EVENT_TYPES.CASE_AWAITING_CENTER,
  ...
  [`${PIPELINE_STAGES.AWAITING_INFO}->${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}`]: CASE_EVENT_TYPES.CASE_OPENED_FOR_APPLICATIONS,
  [`${PIPELINE_STAGES.AWAITING_INFO}->${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}`]: CASE_EVENT_TYPES.CASE_AWAITING_CENTER,
  [`${PIPELINE_STAGES.AWAITING_INFO}->${PIPELINE_STAGES.ARCHIVED}`]: CASE_EVENT_TYPES.CASE_STALE_ARCHIVED,
  [`${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}->${PIPELINE_STAGES.ASSIGNED}`]: CASE_EVENT_TYPES.CASE_ASSIGNED,
  [`${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}->${PIPELINE_STAGES.PENDING_REVIEW}`]: CASE_EVENT_TYPES.CASE_REVIEW_STARTED,
  [`${PIPELINE_STAGES.OPEN_FOR_APPLICATIONS}->${PIPELINE_STAGES.ARCHIVED}`]: CASE_EVENT_TYPES.CASE_DISMISSED,
  [`${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}->${PIPELINE_STAGES.ASSIGNED}`]: CASE_EVENT_TYPES.CASE_CENTER_CONFIRMED,
  [`${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}->${PIPELINE_STAGES.PENDING_REVIEW}`]: CASE_EVENT_TYPES.CASE_UNABLE_TO_ASSIGN,
  [`${PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION}->${PIPELINE_STAGES.ARCHIVED}`]: CASE_EVENT_TYPES.CASE_DISMISSED,
  [`${PIPELINE_STAGES.ASSIGNED}->${PIPELINE_STAGES.ACCEPTED}`]: CASE_EVENT_TYPES.CASE_ACCEPTED,
  [`${PIPELINE_STAGES.ASSIGNED}->${PIPELINE_STAGES.IN_ATTENTION}`]: CASE_EVENT_TYPES.CASE_ATTENTION_STARTED,
...
export const PIPELINE_STAGE_LABELS: Record<PipelineStage, string> = {
  ...
  [PIPELINE_STAGES.PENDING_REVIEW]: 'En revisión',
  [PIPELINE_STAGES.VALIDATING]: 'Validando',
  [PIPELINE_STAGES.AWAITING_INFO]: 'Info pendiente (en revisión)',
  [PIPELINE_STAGES.OPEN_FOR_APPLICATIONS]: 'Esperando cobertura',
  [PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION]: 'Esperando cobertura',
  [PIPELINE_STAGES.ASSIGNED]: 'En progreso',
  [PIPELINE_STAGES.ACCEPTED]: 'En progreso',
  [PIPELINE_STAGES.IN_ATTENTION]: 'En progreso',
...
export const PIPELINE_STAGE_TONES: Record<PipelineStage, string> = {
  ...
  [PIPELINE_STAGES.OPEN_FOR_APPLICATIONS]: 'bg-info/20 text-info',
  [PIPELINE_STAGES.AWAITING_CENTER_CONFIRMATION]: 'bg-warning/20 text-warning',
