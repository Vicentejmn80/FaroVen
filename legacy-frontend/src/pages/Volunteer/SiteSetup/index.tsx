...
import { getGoogleMapsEmbedLink, getGoogleMapsLink } from '@/lib/utils'
...
export function VolunteerSiteSetupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    const shouldRegisterNew = registerNew || !exactMatch
  ...
  const draftMapLink = getGoogleMapsLink({
    name: search.trim(),
    address: newAddress.trim() || null,
  })
  const draftEmbedLink = getGoogleMapsEmbedLink({
    name: search.trim(),
    address: newAddress.trim() || null,
  })
  const selectedSiteMapLink = exactMatch
    ? getGoogleMapsLink({
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {search.trim() && !exactMatch && (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground rounded-lg border border-dashed p-3">
                  No encontramos &quot;{search.trim()}&quot; en la lista. Completa la dirección para que
                  quede enlazado con Google Maps y otros coordinadores puedan ubicarlo rápido.
                </p>
                <div>
                  <label htmlFor="site-address" className="label block mb-1.5">
                    Dirección del {SITE_TYPE_LABELS[siteType].toLowerCase()}
                  </label>
                  <Input
                    id="site-address"
                    placeholder="Ej: Av. San Martín, Caracas"
                    value={newAddress}
                    onChange={(e) => {
                ...
                {draftMapLink && (
                  <div className="space-y-2 rounded-lg border p-3">
                    <p className="text-xs text-muted-foreground">
                      Vista rápida del punto para confirmar antes de guardar:
                    </p>
                    {draftEmbedLink && (
                      <iframe
                    ...
                    <Button type="button" variant="outline" size="sm" asChild>
                      <a href={draftMapLink} target="_blank" rel="noreferrer">
            ...
            {exactMatch && (
              <div className="space-y-2 rounded-lg border p-3">
                ...
                {exactMatch.address && (
                  <p className="text-xs text-muted-foreground">📍 {exactMatch.address}</p>
                )}
                {selectedSiteMapLink && (
                  <Button type="button" variant="outline" size="sm" asChild>
                    <a href={selectedSiteMapLink} target="_blank" rel="noreferrer">
